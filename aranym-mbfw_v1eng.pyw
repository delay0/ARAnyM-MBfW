...
class AranymMidiGUI:
    def __init__(self, root):
        ...
        # Set window size and positioning
    ...
    def open_options_window(self):
        ...
        def save_options():
            ...
            self.append_log(prefix + "⚙️ Configuration and process name successfully updated.")
            options_win.destroy()

        # Creates lower button layout frame
    ...
    # Core background processing stream loop
    def monitor_and_stream_core(self):
        ...
        if not self.emulator.already_running:
            ...
            self.append_log(prefix + "Hooking up asynchronously into existing MIDI stream...")
        ...
        while os.path.exists(midi_path) is False and self.running:
            if self.emulator.is_terminated():
                self.append_log(prefix + "⏹️ Emulator terminated before file creation. Aborting.")
                return
        ...
        while self.running:
            if self.emulator.is_terminated():
                self.append_log(prefix + "🏁 Emulator process terminated. Closing bridge...")
                break
    ...
    # Cleans handles and saves selected device port on exit
    def on_closing(self):
        self.running = False
